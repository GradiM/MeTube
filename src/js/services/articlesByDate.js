...
const articlesByDate = (urlAPI, apiKey) => (sorted, year, callBack) => {
  if (!sorted) {
    ...
    const request = axios.get(`${urlAPI}/discover/movie?api_key=${apiKey}&language=en-EN&sort_by=original_title.${sorted}&include_adult=false&include_video=false&page=1&primary_release_year=${year}`);
    request.then(({ data }) => callBack(data));
  }
};
