...
const articlesByDuration = (urlAPI, apiKey) => (maxMinute, callBack) => {
  const request = axios.get(`${urlAPI}/discover/movie?api_key=${apiKey}&language=en-EN&sort_by=original_title.asc&include_adult=false&include_video=false&page=1&with_runtime.gte=1&with_runtime.lte=${maxMinute}`);
